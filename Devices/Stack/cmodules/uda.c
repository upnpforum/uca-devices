...
static const char **find_action_arguments_names(GUPnPServiceActionInfo *actionInfo)
{
    int i = 0;
    const char **args_names = malloc(MAX_ARG_COUNT * sizeof(char *));
    if (args_names == NULL) {
        return NULL;
    }
...
static const char **find_action_arguments_values(GUPnPServiceAction *action, const char **names)
{
    int i = 0;
    const char **args_values = malloc(MAX_ARG_COUNT * sizeof(char *));
    ...
    for (const char **name = names; *name != NULL; name++) {
        ...
        args_values[i] = (char *)g_value_get_string(value);
        i++;
    }
    return args_values;
}
...
static void free_output(action_output_t output)
{
    ...
    free((void *)output.error_message);
}
                     ...
                     )
{
    ...
    if (result != 0) {
        fprintf(stderr, "* failed to lock mutex, skipping invocation.\n");
        return;
    }
    ...
    if (stack->ahandler != NULL) {
        ...
        fprintf(stderr, "* service found (%s)\n", service_name);

        GUPnPServiceActionInfo *actionInfo
            = (GUPnPServiceActionInfo *)gupnp_service_introspection_get_action(intro, action_name);
        fprintf(stderr, "* action found (%s)\n", action_name);
        ...
        fprintf(stderr, "* SOAP handler missing.");
                             ...
                             )
{
        ...
        )
    {
        ...
        gupnp_service_thaw_notify((GUPnPService *)info);
